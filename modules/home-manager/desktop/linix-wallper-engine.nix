...
let
  ...
  cfg = config.services.linux-wallpaper-engine;

in
{
  meta.maintainers = [ hm.maintainers.ckgxrg ];

  options.services.linux-wallpaper-engine = {
  ...
  config = mkIf cfg.enable {
    assertions = [
      (lib.hm.assertions.assertPlatform "services.linux-wallpaper-engine" pkgs lib.platforms.linux)
    ];
